...
describe("WakeUpPaymaster", () => {
  ...
  describe("Set Up", () => {
    ...
    it("should set the correct owner", async () => {
      ...
      expect(ownerAddress).to.equal(getAddress(testEnv.clients.owner.account.address));
    });
  });
  ...
  describe("Account Management", () => {
    ...
    it("should revert when an not ownerAddress tries to check if account is allowed", async () => {
      await expect(testEnv.paymasterService.isAccountAllowed(testEnv.clients.investorOne.account.address, testEnv.clients.investorOne)).to.be.rejectedWith(/OwnableUnauthorizedAccount/);
    });
    ...
    it("should allow the owner to remove an account", async () => {
      ...
      //Remove the account
      await testEnv.paymasterService.removeAccount(testEnv.clients.investorOne.account.address);

      // Check if account was removed by asking the contract if the account is allowed
      const isAccountRemoved = await testEnv.paymasterService.isAccountAllowed(testEnv.clients.investorOne.account.address);
      expect(isAccountRemoved).to.be.false;
    });

    it("should revert when an not ownerAddress tries to remove an account", async () => {
      await expect(testEnv.paymasterService.removeAccount(testEnv.clients.investorOne.account.address, testEnv.clients.investorOne)).to.be.rejectedWith(/OwnableUnauthorizedAccount/);
    });
  });
  ...
  describe("Validate Paymaster User OP", () => {
    it("should revert when the paymaster user op has no sender", async () => {
      ...
      await expect(testEnv.paymasterService.validatePaymasterUserOp(userOp)).to.be.rejectedWith(/senderNotProvided/);
    });
    ...
    it("should revert when the sender is not allowed", async () => {
      ...
      await expect(testEnv.paymasterService.validatePaymasterUserOp(userOp)).to.be.rejectedWith(/SIG_VALIDATION_FAILED/);
