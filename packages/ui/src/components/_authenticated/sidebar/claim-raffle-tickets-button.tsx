...
export const ClaimRaffleTicketsButton: React.FC<{}> = () => {
  const { claimableTickets, isPending } = useSuperchainRaffle();

  if (claimableTickets === 0 || isPending) {
    return null;
  }
  return (
    <Link to="/profile" className="h-[70px] rounded-lg p-3 flex flex-col justify-between bg-[linear-gradient(-25deg,_#FF6375,_#FF0420,_#FF6375)] text-white">
      ...
      <div className="flex items-center gap-2 ml-6">
        <span className="font-medium text-sm">Claim {claimableTickets} tickets</span>
